...
type WordStatus = { text: string; status: 'waiting' | 'current' | 'correct' | 'incorrect' };
...
const WORD_LIST = [
  ...
  'essayer', 'réussir', 'échouer', 'recommencer', 'terminer', 'abandonner',
  'célébrer', 'féliciter', 'remercier', 'excuser', 'pardonner', 'aider', 'soutenir',
  'conseiller', 'guider', 'diriger', 'commander', 'obéir', 'respecter', 'admirer',
  'détester', 'préférer', 'adorer', 'apprécier', 'supporter', 'tolérer',
  'négocier', 'discuter', 'débattre', 'argumenter', 'convaincre', 'douter',
  'réfléchir', 'méditer', 'observer', 'examiner', 'analyser',
  ...
  'temps', 'année', 'mois', 'semaine', 'weekend',
  ...
  'fruit', 'légume', 'viande', 'dessert', 'repas', 'dîner', 'déjeuner',
  'téléphone', 'ordinateur', 'internet', 'email', 'message', 'photo', 'vidéo', 'musique',
  'livre', 'journal', 'magazine', 'film', 'série', 'émission', 'radio',
...
type Result = { timestamp: number; wpm: number; accuracy: number };
...
export default function TypingSpeed() {
  ...
  useEffect(() => {
    if (timeLeft === 0 && !isFinished) {
      setIsFinished(true);
      setIsStarted(false);
      setFinalPoints(calculatePoints('typingSpeed', { wpm: wordCount }));
  ...
  const shuffleWords = (): WordStatus[] =>
    WORD_LIST.sort(() => Math.random() - 0.5).map((word) => ({
      text: word,
  ...
  const getVisibleLines = () => {
    ...
    for (let i = 0; i < words.length; i += WORDS_PER_LINE) {
      ...
      while (line.length < WORDS_PER_LINE) line.push({ text: '', status: 'waiting' });
      allLines.push(line);
    }
    return allLines.slice(currentLine, currentLine + 2);
  };
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.endsWith(' ')) {
      ...
      if (currentWordIndex + 1 < newWords.length) newWords[currentWordIndex + 1].status = 'current';
      if ((currentWordIndex + 1) % WORDS_PER_LINE === 0) {
        setCurrentLine(Math.floor((currentWordIndex + 1) / WORDS_PER_LINE));
      }
      setWords(newWords);
      if (isCorrect) setWordCount((prev) => prev + 1);
  ...
  const prepareChartData = () => {
    ...
    globalResults.forEach((result) => {
      if (result.wpm && result.wpm > 0) {
        const idx = Math.floor(result.wpm / 10);
        if (idx >= 0 && idx < intervals.length) counts[idx]++;
      }
    });
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: { display: true, text: 'Distribution globale des scores de vitesse de frappe' },
    },
    scales: {
      y: { beginAtZero: true, title: { display: true, text: 'Pourcentage des joueurs (%)' } },
      x: { title: { display: true, text: 'Mots par minute' } },
    },
  };
      ...
      <div className="min-h-screen bg-white dark:bg-gray-900 flex items-center justify-center">
        {!isStarted && !isFinished && showStartModal ? (
          <StartModal
            ...
            description={
              <p className="mb-4">
                Tapez le texte ci-dessous aussi rapidement et précisément que possible. Vous avez
                60 secondes pour taper le maximum de mots.
              </p>
            }
            onStart={handleStart}
            stats={
                  ...
                  <div className="w-full max-w-3xl">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onKeyDown={() => { if (!hasStartedTyping) setHasStartedTyping(true); }}
